...
type CreateTaksRequest struct {
	Title        string  `json:"title" validate:"required,min=1,max=100"          example:"Домашнее задание"`
	Description  *string `json:"description" validate:"omitempty,min=1,max=1000"  example:"Сделать до четверга дз"`
...
// @Description  Создать новую заадачу в системе
// @Tags         tasks
// @Accept       json
// @Produce      json
// @Param        request body CreateTaksRequest true "CreateTask тело запроса"
...
func (h *TasksHTTPHandler) CreateTask(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		responseHandler.ErrorRespons(err, "failed to create task")
