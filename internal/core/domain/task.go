...
func (t *Task) Validate() error {
	...
	if t.Completed {
		...
		if t.CompletedAt.Before(t.CreatedAt) {
			return fmt.Errorf("`CompletedAt` can`t before `CreatedAt`: %w", core_errors.ErrInvalidArgument)
		}
	} else {
		if t.CompletedAt != nil {
...
func (t *Task) ApplyPatch(patch TaskPatch) error {
	if err := patch.Validate(); err != nil {
		return fmt.Errorf("valedate task patch:%w", err)
	}
	...
	if err := tmp.Validate(); err != nil {
		return fmt.Errorf("valedate patched task: %w", err)
	}
